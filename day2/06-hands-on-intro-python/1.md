...
1. In VSCode, from the menu at the top of the screen, click `View`.  When the menu appears, click `Terminal` to open a new terminal window at the bottom of the screen.

2. In the Terminal window, enter the Python interactive shell by typing `python -i` and pressing `Enter`.
...
3. Type `print("Hello, World!")` in the Python interactive shell and press `Enter`.
...
4. Type `exit()` and press `Enter` to exit the Python interactive shell.

5. With the `~/code/CiscoU.-Workshop/` folder already open in the panel on the left of the screen:
    - Navigate to the directory for this lab (`day2/06-hands-on-intro-python`) and right-click on the `code` folder, which is where you will create your Python script.
    - Choose `New File` from the context menu.
    - Type `hello.py` to name your file, and press `Enter`.

6. The `hello.py` file should open automatically, but you can also open it by clicking on it in the Explorer on the left.

7. In the `hello.py` file, type `print("Hello, World!")` and save it by selecting `File > Save` or by using the keyboard shortcut.

8. Run your Python script by typing `python hello.py` in the Terminal and pressing `Enter`.
    ...
    > `pwd` ->`ls` -> `cd day2` -> `cd 06-hands-on-intro-python` -> `cd code`
...
5. Enter the following into the Python interactive shell to print the variables to the screen.
    ...
    > Note: After a multiline statement, such as the print statements above, the Python interactive shell may require you to press `ENTER` multiple times to execute all the lines. You might also get a warning message from VSCode if you attempt to paste the entire code block. Click `Paste` to proceed.
...
- **Community developed libraries** can be installed from [Python Package Index (PyPI)](https://pypi.org/). These 3rd party libraries can be installed with the Python tool `pip`. The Python interactive shell can access them if they are installed withing the same environment (virtual or global).
...
No clean up or lab teardown is required.  We will be using this same environment for the network automation portion using Netmiko.
